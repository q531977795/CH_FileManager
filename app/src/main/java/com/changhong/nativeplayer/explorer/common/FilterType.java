...
public class FilterType {
	public static void filterType(CommonActivity ctx) {
		...
		if (share.getBoolean("audioFlag", true)) {
			Editor shareAudio = ctx.getSharedPreferences(
		...
		//DOC conditions add by zhongpujun 160318
		if(share.getBoolean("docFlag", true))
		{
			Editor shareDoc = ctx.getSharedPreferences(
					"DOC", Context.MODE_WORLD_READABLE).edit();
			shareDoc.putString("TXT", "TXT");
			shareDoc.putString("DOC", "DOC");
			shareDoc.putString("PPT", "PPT");
			shareDoc.putString("PDF", "PDF");	
			shareDoc.putString("XLSX", "XLSX");
			shareDoc.putString("DOCX", "DOCX");
			editor.putBoolean("docFlag", false);
			editor.commit();
			shareDoc.commit();
		}
