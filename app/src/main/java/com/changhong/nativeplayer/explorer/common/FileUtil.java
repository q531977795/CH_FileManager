...
import com.changhong.nativeplayer.explorer.activity.SambaActivity;
...
public class FileUtil {
    ...
    //add by zhongpujun
    //doc file collection
    private List<File> docList = null;

    //other file collection
    private List<File> otherList = null;
    ...
    public static String getMIMEType(File f, CommonActivity mea) {
        ...
        //read doc condition
        //CNcomment: 读取文档条件
        SharedPreferences shareDoc = mea.getSharedPreferences("DOC",
                Context.MODE_WORLD_READABLE);
        String strDoc = shareDoc.getString(end, "");
     ...
     *
     * @param mContext
     * @param res      corresponding strings resources file CNcomment: 对应的Strings资源文件
     */
    ...
    private void getFiltList(String local) {
        ...
        // doc list
            ...
            getDoc(currentFilePath, local);
        }
        // other list
        else if (filterCount == 5) {
            otherList = new ArrayList<File>();
            getOther(currentFilePath, local);
        }
    }
    ...
    /**
     * get picture collection directory
     *
     * @param fileRoot
     * @return
     */
    ...
    /**
     * get doc collection directory
     *
     * @param fileRoot
     * @return
     */
    private void getDoc(String fileRoot, String local) {
        // determine the search results is empty
        // CNcomment:判断搜索结果是否为空
        if (fileDir.size() > 0 || file.size() > 0) {
            return;
    ...
    /**
     * get other collection directory
     *
     * @param fileRoot
     * @return
     */
    private void getOther(String fileRoot, String local) {
        // determine the search results is empty
        // CNcomment:判断搜索结果是否为空
        if (fileDir.size() > 0 || file.size() > 0) {
            return;
        ...
        synchronized (mx.lock) {
            ...
            for (int i = 0; i < le.size(); i++) {
                if (SystemProperties.get("ro.product.target").equals("telecom")
                        || SystemProperties.get("ro.product.target").equals("unicom")) {
                    File item = le.get(i);
                    if (item != null) {
